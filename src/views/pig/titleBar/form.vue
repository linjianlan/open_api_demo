<template>
    <el-dialog :title="form.tId ? '编辑' : '新增'" v-model="visible"
...
import { getObj, addObj, putObj } from '/@/api/pig/titleBar'
...
const dataRules = ref({
    ...
    tParentId: [{required: true, message: '父级id不能为空', trigger: 'blur'}, { validator: rule.number, trigger: 'blur' }],
        tUrl: [{required: true, message: '跳转路径不能为空', trigger: 'blur'}],
        tIcon: [{required: true, message: '图标或图片路径不能为空', trigger: 'blur'}],
    tGrade: [{required: true, message: '级别 1 父级    1++...子级不能为空', trigger: 'blur'}, { validator: rule.number, trigger: 'blur' }],
})
